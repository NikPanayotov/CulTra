...
if (isset($_GET['input']) && isset($_GET['item_order']) && isset($_GET['object']) && isset($_GET['cycle'])) {
	...
	if (preg_match($regex, $input)) {
		...
		$sql = "SELECT input FROM transmissions WHERE node_id=" . $db->quote($_SESSION['cultra']['parent_id']) . " AND object=". $db->quote($object) . " AND section='TESTING';";
		...
		echo json_encode(array('correct' => $correct, 'cycle' => $_SESSION['cultra']['cycle'], 'items_correct' => intval($_SESSION['cultra']['items_correct']), 'item_total' => intval($_SESSION['cultra']['item_total']), 'item_order' => intval($_SESSION['cultra']['item_order']), 'object' => $object, 'required' => $_SESSION['cultra']['pass_requirement']));
