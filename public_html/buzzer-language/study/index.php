<?php
	...
	$sql_select = "SELECT * FROM transmission_sessions WHERE session_number=" . strval($_SESSION['cultra']['session_number']) . ";";
	$result_rows = $db->select($sql_select);
	if ($result_rows->num_rows > 0) {
		if ($row = $result_rows->fetch_assoc()) {
			$_SESSION['cultra']['state'] = $row['progress'];
			stateMachine();
		}
	} else {
	...
	function stateTransition($current_state, $next_state) {
		if ($_SESSION['cultra']['item_order'] > $_SESSION['cultra']['item_total']) {
	...
	function stateMachine() {
		...
		switch($_SESSION['cultra']['state']) {
			case "START":
				stateTransition('pages/before_training.html', 'COMPREHENSION_TRAINING');
				break;
			case "COMPREHENSION_TRAINING":
				if ($_SESSION['cultra']['training_score'] >= $_SESSION['cultra']['training_length_condition']) {
	...
	function updateState($state) {
		...
		if (isset($_SESSION['cultra']['initialised_state'])) {
			unset($_SESSION['cultra']['initialised_state']);
		}
		
		// connect to db
		$db = new Db();
		
		$_SESSION['cultra']['state'] = $state;
		// Update progress
		$sql_update = "UPDATE transmission_sessions SET progress=" . $db->quote($_SESSION['cultra']['state']) . " WHERE session_number=" . strval($_SESSION['cultra']['session_number']) . ";";
	...
	function ending() {
		...
		$sql_update = "UPDATE transmission_sessions SET end_timestamp = current_timestamp WHERE session_number=" . $db->quote($_SESSION['cultra']['session_number']) . ";";
		if ($db->query($sql_update) === TRUE) {
			//
		} else {
			$db->query("ROLLBACK;");
		...
		$sql_update = "UPDATE nodes SET node_type = 'nonterminal' WHERE node_id=" . $db->quote($_SESSION['cultra']['parent_id']) . ";";
		if ($db->query($sql_update) === TRUE) {
			//
		} else {
			$db->query("ROLLBACK;");
		...
		// Free the node
		$sql_update = "UPDATE nodes SET status = 'free' WHERE node_id=" . $db->quote($_SESSION['cultra']['parent_id']) . ";";
		if ($db->query($sql_update) === TRUE) {
			//
		} else {
			$db->query("ROLLBACK;");
		...
		/*if ($_SESSION['cultra']['generation'] > 10) {
			// last node in chain
			$result = $db -> query("INSERT INTO nodes (session_number, parent_id, generation, status, node_type, tree) VALUES (" . $db->quote($_SESSION['cultra']['session_number']) . "," . $db->quote($_SESSION['cultra']['parent_id']) . "," . $db->quote($_SESSION['cultra']['generation']) . ","  . "'free', 'endbranch'"  . "," . $_SESSION['cultra']['tree'] . ")");
		} else {*/
			// or continue the chain
			$result = $db -> query("INSERT INTO nodes (session_number, parent_id, generation, status, node_type, tree) VALUES (" . $db->quote($_SESSION['cultra']['session_number']) . "," . $db->quote($_SESSION['cultra']['parent_id']) . "," . $db->quote($_SESSION['cultra']['generation']) . ","  . "'free', 'terminal'"  . "," . $_SESSION['cultra']['tree'] . ")");	
		//}
		
		$sql_id = $db->getInsertId();
		
		// Update session to say correct node id
		$sql_update = "UPDATE transmission_sessions SET node_id = ". $db->quote($sql_id) . "WHERE session_number=" . $db->quote($_SESSION['cultra']['session_number']) . ";";
		if ($db->query($sql_update) === TRUE) {
			//
		} else {
			$db->query("ROLLBACK;");
		...
		$sql_update = "UPDATE transmissions SET node_id = ". $db->quote($sql_id) . "WHERE session_number=" . $db->quote($_SESSION['cultra']['session_number']) . ";";
		if ($db->query($sql_update) === TRUE) {
			//
		} else {
			$db->query("ROLLBACK;");
		...
		$code = str_pad($_SESSION['cultra']['session_number'], 5, "0", STR_PAD_LEFT);
		...
		$sql_update = "UPDATE transmission_sessions SET completion_code = '$code' WHERE session_number=" . $db->quote($_SESSION['cultra']['session_number']) . ";";
		if ($db->query($sql_update) === TRUE) {
			//
		} else {
			echo "Error updating record: " . $db->error();	
